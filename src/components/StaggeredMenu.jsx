...
const styles = `
.staggered-menu { min-height: 100%; position: relative; overflow: hidden; background: #000; color: #f6f6ef; }
.staggered-menu__toggle { position: absolute; z-index: 8; right: clamp(18px, 3vw, 34px); top: clamp(18px, 3vw, 30px); width: 72px; height: 72px; border: 1px solid #f6f6ef; border-radius: 0; background: #000; color: #f6f6ef; font: 600 10px/1 "DM Sans", sans-serif; cursor: pointer; transition: transform 350ms cubic-bezier(.16,1,.3,1), background 220ms ease, color 220ms ease; }
.staggered-menu__toggle:hover { background: #f6f6ef; color: #000; transform: rotate(4deg); }
.staggered-menu.is-open .staggered-menu__toggle { background: #f6f6ef; color: #000; transform: rotate(-4deg); }
.staggered-menu__overlay { position: absolute; z-index: 6; inset: 0; display: grid; grid-template-columns: minmax(0, 1.2fr) minmax(240px, .8fr); gap: clamp(26px, 5vw, 70px); padding: clamp(86px, 10vh, 112px) clamp(24px, 4vw, 52px) clamp(28px, 4vw, 40px); background: #000; color: #f6f6ef; clip-path: ellipse(0% 0% at 100% 0%); transition: clip-path 820ms cubic-bezier(.76,0,.24,1); pointer-events: none; }
...
.staggered-menu__link span { color: #f6f6ef; font-size: 24px; opacity: 0; transform: translateX(-12px); transition: opacity 220ms ease, transform 350ms cubic-bezier(.16,1,.3,1); }
...
.staggered-menu.is-compact { background: #000; }
.staggered-menu.is-compact .staggered-menu__overlay { grid-template-columns: 1fr; align-content: end; background: #000; color: #f6f6ef; clip-path: inset(0 0 100% 0); }
.staggered-menu.is-compact.is-open .staggered-menu__overlay { clip-path: inset(0); }
.staggered-menu.is-compact .staggered-menu__nav { width: min(920px, 100%); align-self: end; border-color: #44463f; }
.staggered-menu.is-compact .staggered-menu__link { min-height: clamp(70px, 10vh, 94px); border-color: #44463f; }
.staggered-menu.is-compact .staggered-menu__link strong { font-size: clamp(42px, 6vw, 76px); }
.staggered-menu.is-compact .staggered-menu__link span { color: #f6f6ef; }
...
@media (max-width: 700px) {
  ...
  .staggered-menu.is-compact .staggered-menu__link { grid-template-columns: 1fr auto; min-height: 68px; }
}
@media (prefers-reduced-motion: reduce) {
  .staggered-menu__overlay, .staggered-menu__link, .staggered-menu__visual, .staggered-menu__image { transition: none; }
}
`;
