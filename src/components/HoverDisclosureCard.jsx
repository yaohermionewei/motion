...
.disclosure-card:focus-visible { outline: 2px solid #f5f6f1; outline-offset: 5px; }
...
.disclosure-card__cover-title { position: absolute; z-index: 2; right: 24px; bottom: 22px; left: 24px; color: #f5f6f1; }
.disclosure-card__cover-title strong { max-width: 11ch; font: 400 clamp(42px, 7vw, 76px)/.82 "Instrument Serif", serif; }
.disclosure-card__panel { position: absolute; z-index: 3; inset: 0; display: grid; align-content: end; padding: clamp(26px, 5vw, 52px); color: #f5f6f1; opacity: 0; transform: translateY(26px); transition: opacity 300ms ease, transform 650ms cubic-bezier(.16,1,.3,1); }
...
.disclosure-card__description { display: block; max-width: 50ch; margin: 16px 0 0; color: rgba(245,246,241,.72); font: 500 clamp(13px, 1.6vw, 17px)/1.45 "DM Sans", sans-serif; }
...
.disclosure-card.is-active .disclosure-card__cover-title { opacity: 0; }
  ...
  return (
    <section className="disclosure-card-demo">
      ...
      >
        ...
        <span className="disclosure-card__cover-title"><strong>{title}</strong></span>
